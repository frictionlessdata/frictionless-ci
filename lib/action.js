...
export async function action({
  ...
  packageGlob = config.PACKAGE_GLOB,
  resourceGlob = config.RESOURCE_GLOB,
  tableGlob = config.TABLE_GLOB,
} = {}) {
  let inquiry
  let report
  ...
  if (!inquiry) {
    try {
      const packages = await globby(packageGlob)
      const resources = await globby(resourceGlob)
      const tables = await globby(tableGlob)
      if (packages.length) {
        inquiry = { tasks: packages.map((descriptor) => ({ descriptor })) }
      } else if (resources.length) {
        inquiry = { tasks: resources.map((descriptor) => ({ descriptor })) }
      } else if (tables.length) {
        inquiry = { tasks: tables.map((path) => ({ path })) }
      } else {
        throw new Error('no sources to validate')
      }
    } catch (error) {
