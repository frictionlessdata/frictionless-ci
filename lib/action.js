...
exports.action = async function () {
  ...
  if (await fs.pathExists(config.CONFIG_FILE)) {
    try {
      ...
      if (!conf.hasOwnProperty(name)) throw new Error(`not existent "${name}"`)
      inquiry = conf[name]
    } catch (error) {
  ...
  if (!inquiry) {
    try {
      ...
      // NOTE: it might be better to use "path/descriptor" properties. It requries:
      // https://github.com/frictionlessdata/frictionless-py/issues/874
  ...
  try {
    ...
    core.setFailed(`Cannot validate inquiry: ${error.message}`)
    return
  }
